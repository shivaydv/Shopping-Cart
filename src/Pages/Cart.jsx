...
export default function Cart() {
  ...
  return (
    <div className={`flex max-md:flex-col  container mx-auto min-h-screen  `}>
      <section className="md:w-3/4  h-[90vh]  p-6 w-full overflow-y-scroll">
        ...
        <div className="flex flex-col gap-4 md:gap-2 h-full ">
          {cartItems.map((item) => (
            // Product card
            <div
              className="flex max-md:flex-col max-md:items-start max-md:space-y-2 justify-between items-center "
              key={item.id}
            >
          ...
          <div className="flex items-center justify-center py-2">
            {/* Buttun to Add or Remove Product from cart  */}
            {cartItems.length > 0 ? (
              <button
                onClick={clearCart}
                className="bg-red-600 max-md:mt-4 p-2 rounded-lg text-white font-semibold"
              >
                Remove All
              </button>
            ) : (
              <div className="font-semibold gap-6 flex flex-col items-center justify-center max-md:h-[50vh] h-[60vh] ">
                ...
                <Link
                  to={"/products"}
                  className="bg-black p-2 rounded-lg text-white font-semibold"
                >
                  Add Products
                </Link>
              </div>
      ...
      {/* Sidebar With Total Cost of Products In The Cart  */}
      <aside className="md:w-1/4 h-full md:shadow-xl max-md:pt-0 p-6 md:fixed md:right-0 ">
        <h1
          className="text-2xl font-bold trac
        king-wide"
        >
          Summary
        </h1>
        ...
        {cartItems.length > 0 && (
          <button className="w-full bg-black p-2 rounded-lg text-white font-semibold tracking-wide mt-10">
            Buy Now
          </button>
        )}
      </aside>
